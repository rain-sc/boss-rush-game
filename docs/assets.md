...
- **固定高度、寬度填滿**:以邏輯高度(270)為基準縮放(`scale = 螢幕高 / 270`),寬度依螢幕比例動態延伸填滿,**兩側無黑邊**;像素維持正方(搭配 nearest 取樣)。16:9 螢幕的可視寬度剛好回到 480。超寬螢幕看到較多水平場地。
- **橫向為主**:遊戲為橫向設計;手機**直向時顯示「請旋轉為橫向」提示**(`@media (orientation: portrait) and (pointer: coarse)`)。全螢幕鈕僅在觸控裝置顯示(桌機已填滿視窗,不需要)。
