...
- **fit 填滿縮放**:在保持 16:9 比例下盡量填滿螢幕(letterbox 補邊),確保整個場地完整可見。16:9 全螢幕(如 1920×1080)會自然落在整數倍;其餘尺寸採非整數 fit 以避免畫面過小 / 溢出(搭配 nearest 取樣仍維持像素感)。
- **橫向為主**:遊戲為橫向設計;手機**直向時顯示「請旋轉為橫向」提示**(`@media (orientation: portrait) and (pointer: coarse)`),引導到橫向以獲得最大畫面。
